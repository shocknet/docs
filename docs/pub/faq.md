...
Delete `~/lightning_pub/admin.npub` and restart Pub to generate new `admin.connect` / `admin.enroll` material. Only one admin exists; the old admin becomes a guest. See [Reset admin access](./admin-reset.md).
